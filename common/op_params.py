...
class opParams:
  def __init__(self, calling_function=''):
    ...
    self.fork_params = {      
      ...
      'TUNE_LAT_TRX_friction_error_rate_factor': Param(0.3, float, 'Friction is not allowed to contribute to steer error, and it is preemptive reduced before this occurs based on the error rate. Here you can increase/decrease the error rate used in this calculation so that friction is reduced earlier/later before friction begins to contribute to error. If set too high, false positives will become an issue and you\'ll be reducing friction when you shouldn\'t have.', live=True, min_val=0.1, max_val=10.0, show_op_param='TUNE_LAT_type', show_op_param_check_val='torque'),
      
      'TUNE_LAT_TRX_friction_smoothing_factor': Param(1.0, float, 'Adjust the smoothing of friction\n', min_val=0.0, max_val=100.0),
