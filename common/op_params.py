...
class opParams:
  def __init__(self, calling_function='', check_for_reset=False, overwrite_params=False):
    ...
    self.fork_params = {      
      ...
      'FP_L1P_buffer_s': Param(0.0, float, 'If extended radar capabilities are enabled, then OpenPilot will consider the car in front of the lead (i.e. the "lead+1") when determining the follow distance to use between you and the lead car. Put simply, a lead that is following *their* lead too closely may suddenly slam on the brakes or fail to react in time, leading to a collision, so it\'s a good idea to follow such a dangerous lead from farther back. So, if the lead is too close to the lead+1 then OpenPilot\'s follow distance is "penalized" (increased). This works by comparing the lead\'s follow distance behind the lead+1 to OpenPilot\'s current desired follow distance. The lead should be maintaining at least the same follow distance as what OpenPilot itself wants behind the lead (actually, the distance used for comparison is the next closest follow profile, so if you\'re using medium follow, then the lead must be at the close-follow distance from the lead+1). If the lead\'s follow distance (fd_lead) is less than OpenPilot\'s desired follow distance (fd) then we will add a "penalty" to OpenPilot\'s follow distance. The follow distance penalty is the difference plus the amount set configured here (the "buffer"), i.e. `fd - fd_lead + buffer`. \n\nHere you set the value of the buffer. So if this setting is set to 0.0, then only the difference will be added to the follow distance. If this setting is positive, then the follow distance will increase more so, and if the setting is negative, less of a follow distance penalty will be applied. Set to minimum value to cancel the follow distance penalty altogether.', live=True, min_val=-3.0, max_val=1.5, unit='seconds follow distance'),
      
      'FP_L1P_smoothing_up': Param(4.0, float, 'If extended radar capabilities are enabled, then OpenPilot will consider the car in front of the lead (i.e. the "lead+1") when determining the follow distance to use between you and the lead car. Put simply, a lead that is following *their* lead too closely may suddenly slam on the brakes or fail to react in time, leading to a collision, so it\'s a good idea to follow such a dangerous lead from farther back. So, if the lead is too close to the lead+1 then OpenPilot\'s follow distance is "penalized" (increased). This works by comparing the lead\'s follow distance behind the lead+1 to OpenPilot\'s current desired follow distance. The lead should be maintaining at least the same follow distance as what OpenPilot itself wants behind the lead (actually, the distance used for comparison is the next closest follow profile, so if you\'re using medium follow, then the lead must be at the close-follow distance from the lead+1). If the lead\'s follow distance (fd_lead) is less than OpenPilot\'s desired follow distance (fd) then we will add a "penalty" to OpenPilot\'s follow distance. \n\nThe penalty changes in a smooth way, and the amount of smoothing is different when the penalty is increasing vs when it is decreasing. Here, you set the smoothing factor for when the penalty is increasing. A lower value is less smoothing (faster response). In general, you want the penalty to respond more quickly when it\'s increasing, so that OpenPilot isn\'t caught off-guard should the lead slam on the brakes or hit the lead+1. Set to zero to disable smoothing completely.', live=True, min_val=0.0, max_val=50.0),
