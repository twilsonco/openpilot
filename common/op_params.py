...
class opParams:
  def __init__(self, calling_function=''):
    ...
    self.fork_params = {      
      ...
      'TUNE_LAT_TRX_ki_e': Param(20.0, float, "This fork uses an \"autotuned\" PID controller, where the kp, ki, and kd values change based on the rate of change of controller error (actually the output, but that's based on the error). This controls how much ki is able to change.", live=True, min_val=0.0, max_val=1000.0, show_op_param='TUNE_LAT_type', show_op_param_check_val='torque'),
      
      'TUNE_LAT_TRX_kd_e': Param(10.0, float, "This fork uses an \"autotuned\" PID controller, where the kp, ki, and kd values change based on the rate of change of controller error (actually the output, but that's based on the error). This controls how much kd is able to change.", live=True, min_val=0.0, max_val=1000.0, show_op_param='TUNE_LAT_type', show_op_param_check_val='torque'),
    ...
    self._to_reset = {
      ...
      '2023/03/19-02:00': ['TUNE_LAT_TRX_friction',
                           r'.*low_speed_factor_.*'],
      '2023/03/29-12:00': [r'TUNE_LAT_TRX_.*'],
