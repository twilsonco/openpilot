...
class opParams:
  def __init__(self, calling_function=''):
    ...
    self.fork_params = {      
      ...
      'LP_path_offset_m': Param(-0.04 if TICI else 0.0, float, 'Adjust your default path position. Increase it to move your position to the left, or decrease it to move to the right. This is like camera offset, but applies to lateral position when no lanelines are present or they are not being used.\n', live=True, min_val=-0.5, max_val=0.5, is_common=True, unit='meters'),
      ...
      'TUNE_LAT_TRX_friction_power': Param(2.0, float, 'Friction is the lateral jerk component of feedforward. It depends on desired lateral jerk according to the power you set here. It is computed such that the `friction_v` values are the max possible values despite this power value.', live=True, min_val=0.01, max_val=5.0, show_op_param='TUNE_LAT_type', show_op_param_check_val='torque'),
      
      'TUNE_LAT_TRX_friction_factor_bp': Param([0.7, 3.0], [list, float], 'Friction (lateral jerk feedforward) can conflict with lateral acceleration feedforward if they are both high. The current workaround is to scale down friction based on the current value of lateral acceleration. This controls the lateral acceleration values used.', live=True, min_val=0.0, max_val=10.0, show_op_param='TUNE_LAT_type', show_op_param_check_val='torque', unit="m/s²"),
      
      'TUNE_LAT_TRX_friction_factor_v': Param([1.0, 0.5], [list, float], 'Friction (lateral jerk feedforward) can conflict with lateral acceleration feedforward if they are both high. The current workaround is to scale down friction based on the current value of lateral acceleration. This controls the values of the resulting friction scaling factor corresponding to the counterpart lateral acceleration values.', live=True, min_val=0.0, max_val=1.0, show_op_param='TUNE_LAT_type', show_op_param_check_val='torque'),
      
      'TUNE_LAT_TRX_friction_error_scale': Param(0.5, float, 'Even though lateral jerk FF and lateral acceleration FF (friction) can conflict, if the friction is in the same direction (sign) as the error, we\'ll take it. This value is used to scale the error, which is then added to the friction factor (above) in order to increase the friction factor back to 1.0. That is, we do not increase or decrease the value of friction based on error, but instead we use error to prevent from decreasing the friction value when we otherwise would have based on lateral acceleration alone.', live=True, min_val=0.0, max_val=5.0, show_op_param='TUNE_LAT_type', show_op_param_check_val='torque'),
      
      'TUNE_LAT_TRX_friction_error_rate_scale': Param(0.2, float, 'Same as TUNE_LAT_TRX_friction_error_scale, but now we\'ve avoiding decreasing friction based on agreement between the sign of friction and error rate. The idea being that the otherwise unwanted extra friction is also desirable if it will push against a (e.g.) increasing error rate.', live=True, min_val=0.0, max_val=5.0, show_op_param='TUNE_LAT_type', show_op_param_check_val='torque'),
      ...
      'TUNE_LAT_TRXINDI_roll_compensation': Param(0.0, float, 'Scale the amount of roll compensation for the torque indi controller\n', live=True, min_val=0.0, max_val=10.0, show_op_param='TUNE_LAT_type', show_op_param_check_val='torqueindi'),
      ...
      'TUNE_LAT_TRXINDI_friction': Param(0.05, float, 'Scale the amount of friction for the torque lqr controller. Friction applies additional steer output linearly based on the amount of desired lateral jerk (rate of change of desired lateral acceleration)\n', live=True, min_val=0.0, max_val=1.0, show_op_param='TUNE_LAT_type', show_op_param_check_val='torqueindi'),
      ...
      'TUNE_LAT_TRXLQR_roll_compensation': Param(0.0, float, 'Scale the amount of roll compensation for the torque lqr controller\n', live=True, min_val=0.0, max_val=10.0, show_op_param='TUNE_LAT_type', show_op_param_check_val='torquelqr'),
