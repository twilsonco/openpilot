...
void generic_rx_checks(bool stock_ecu_detected) {
  ...
  // PFEIFER - MADS {{
  if (gas_pressed && !gas_pressed_prev && !(alternative_experience & ALT_EXP_ENABLE_MADS)) {
  ...
  // PFEIFER - MADS {{
  if (brake_pressed && (!brake_pressed_prev || vehicle_moving) && !(alternative_experience & ALT_EXP_MADS_DISABLE_DISENGAGE_LATERAL_ON_BRAKE)) {
    lateral_controls_allowed = 0;
  }
  // }} PFEIFER - MADS
  ...
  // PFEIFER - MADS {{
  if (regen_braking && (!regen_braking_prev || vehicle_moving) && !(alternative_experience & ALT_EXP_MADS_DISABLE_DISENGAGE_LATERAL_ON_BRAKE)) {
    lateral_controls_allowed = 0;
  }

  // If mads is not enabled we set lateral_controls_allowed to controls_allowed to maintain stock behavior
