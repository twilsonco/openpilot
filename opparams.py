...
class opEdit:  # use by running `python /data/openpilot/op_edit.py`
  def __init__(self, restart=False, restart_immediate=False):
    ...
    if restart or restart_immediate:
      ...
      return
    self.op_params = opParams(calling_function="opparams editor", check_for_reset=True, overwrite_params=True)
    self.params = None
  ...
  def run_loop(self):
    while True:
      ...
      n_pad = max([len(i) for i in self.params]) if len(self.params) > 0 else 0
      n_pad_idx = int(log10(len(self.params) + (0 if self.section != '' else len(sections))))+2
      n_pad_vals = max([len(str(i)) for i in values_list])+1 if len(values_list) > 0 else 0
