...
class Events:
    ...
    def clear(self) -> None:
        self.events_prev = {
            k: (v + 1 if k in self.events else 0) for k, v in self.events_prev.items()
        }
...
AlertCallbackType = Callable[
    [car.CarParams, car.CarState, messaging.SubMaster, bool, int], Alert
]


def soft_disable_alert(alert_text_2: str) -> AlertCallbackType:
    print("soft_diable_alert:", alert_text_2)
...
def below_engage_speed_alert(
    ...
    return NoEntryAlert(
        f"Drive above {get_display_speed(CP.minEnableSpeed, metric)} to engage"
    )


def below_steer_speed_alert(
    CP: car.CarParams,
    CS: car.CarState,
    sm: messaging.SubMaster,
    metric: bool,
...
def calibration_incomplete_alert(
    CP: car.CarParams,
    CS: car.CarState,
    sm: messaging.SubMaster,
    metric: bool,
    ...
    first_word = (
        "Recalibration"
        if sm["liveCalibration"].calStatus
...
def out_of_space_alert(
    ...
    full_perc = round(100.0 - sm["deviceState"].freeSpacePercent)
    return NormalPermanentAlert("Out of Storage", f"{full_perc}% full")


def posenet_invalid_alert(
    CP: car.CarParams,
    CS: car.CarState,
    sm: messaging.SubMaster,
    metric: bool,
...
def camera_malfunction_alert(
    ...
    bad_cams = [
        s.replace("State", "")
        for s in all_cams
        if s in sm.data.keys()
        and not sm.all_checks(
            [
                s,
            ]
        )
...
def overheat_alert(
    ...
    cpu = max(sm["deviceState"].cpuTempC, default=0.0)
    gpu = max(sm["deviceState"].gpuTempC, default=0.0)
...
def low_memory_alert(
    ...
    return NormalPermanentAlert(
        "Low Memory", f"{sm['deviceState'].memoryUsagePercent}% used"
    )


def high_cpu_usage_alert(
    CP: car.CarParams,
    CS: car.CarState,
    sm: messaging.SubMaster,
    metric: bool,
    ...
    x = max(sm["deviceState"].cpuUsagePercent, default=0.0)
...
def modeld_lagging_alert(
    ...
    return NormalPermanentAlert(
        "Driving Model Lagging", f"{sm['modelV2'].frameDropPerc:.1f}% frames dropped"
    )


def wrong_car_mode_alert(
    CP: car.CarParams,
    CS: car.CarState,
    sm: messaging.SubMaster,
    metric: bool,
...
EVENTS: dict[int, dict[str, Alert | AlertCallbackType]] = {
    ...
    EventName.dashcamMode: {
        ET.PERMANENT: NormalPermanentAlert("Dashcam Mode", priority=Priority.LOWEST),
    },
    EventName.invalidLkasSetting: {
        ET.PERMANENT: NormalPermanentAlert(
            "Stock LKAS is on", "Turn off stock LKAS to engage"
        ),
    },
    ...
    EventName.carUnrecognized: {
        ET.PERMANENT: NormalPermanentAlert(
            "Dashcam Mode", "Car Unrecognized", priority=Priority.LOWEST
        ),
    },
    EventName.stockAeb: {
        ET.PERMANENT: Alert(
    ...
    EventName.pedalPressed: {
        ...
        ET.NO_ENTRY: NoEntryAlert(
            "Pedal Pressed", visual_alert=VisualAlert.brakePressed
        ),
    },
    ...
    EventName.calibrationInvalid: {
        ...
        ET.SOFT_DISABLE: soft_disable_alert(
    ...
    EventName.roadCameraError: {
        ET.PERMANENT: NormalPermanentAlert(
            "Camera CRC Error - Road", duration=1.0, creation_delay=30.0
        ),
    },
    EventName.wideRoadCameraError: {
        ET.PERMANENT: NormalPermanentAlert(
            "Camera CRC Error - Road Fisheye", duration=1.0, creation_delay=30.0
        ),
    },
    EventName.driverCameraError: {
        ET.PERMANENT: NormalPermanentAlert(
            "Camera CRC Error - Driver", duration=1.0, creation_delay=30.0
        ),
    },
    ...
    EventName.relayMalfunction: {
        ...
        ET.PERMANENT: NormalPermanentAlert(
            "Harness Relay Malfunction", "Check Hardware"
        ),
    ...
    EventName.vehicleSensorsInvalid: {
        ...
        ET.PERMANENT: NormalPermanentAlert(
            "Vehicle Sensors Calibrating", "Drive to Calibrate"
        ),
    ...
    EventName.trafficSignChanged: {
        ET.WARNING: Alert(
            ...
            Priority.LOW,
            VisualAlert.none,
            AudibleAlert.trafficSignChanged,
            1.0,
        ),
    },
    EventName.autoHold: {
...
if __name__ == "__main__":
    ...
    alerts_by_type: dict[str, dict[Priority, list[str]]] = defaultdict(
    ...
    for et, priority_alerts in alerts_by_type.items():
        all_alerts[et] = sorted(
            priority_alerts.items(), key=lambda x: x[0], reverse=True
        )
