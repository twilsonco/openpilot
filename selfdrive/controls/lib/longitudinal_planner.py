...
class LongitudinalPlanner:
  def __init__(self, CP, init_v=0.0, init_a=0.0):
    ...
    # PFEIFER - ETSC {{
    self.experimental_turn_controller = m['experimental_turn_controller'].ExperimentalTurnController(self.VM)
    # }} PFEIFER - ETSC
    # PFEIFER - VTSC {{
    self.vision_turn_controller = m['vision_turn_controller'].VisionTurnController(self.VM)
    # }} PFEIFER - VTSC
    # PFEIFER - MTSC {{
    self.map_turn_controller = m['map_turn_controller'].MapTurnController(self.VM)
    # }} PEFIFER - MTSC
  ...
  def update(self, sm, lateral_planner):
    ...
    # PFEIFER - SPEED {{
    v_cruise = m['speed_limit'].slc.get_max(v_cruise)
    # }} PFEIFER - SPEED

    # PFEIFER - MTSC {{
    enabled = not reset_state and self.CP.openpilotLongitudinalControl
    self.map_turn_controller.update(enabled, v_ego, self.a_desired, v_cruise, sm)
    if self.map_turn_controller.active:
      original_v_cruise = v_cruise
      a_target, v_cruise = self.map_turn_controller.plan
    ...
    # PFEIFER - VTSC {{
    enabled = not reset_state and self.CP.openpilotLongitudinalControl
    self.vision_turn_controller.update(enabled, v_ego, self.a_desired, v_cruise, sm, lateral_planner)
    if self.vision_turn_controller.active:
      original_v_cruise = v_cruise
      a_target, v_cruise = self.vision_turn_controller.plan
      if v_cruise < v_ego and original_v_cruise > v_cruise:
        accel_limits_turns[0] = min(accel_limits_turns[0], a_target - 0.05)
        accel_limits_turns[1] = min(accel_limits_turns[1], a_target)
    # }} PFEIFER - VTSC
    # PFEIFER - CMS {{
    m['current_max_speed'].cms.max_speed = v_cruise
    # }} PFEIFER - CMS
