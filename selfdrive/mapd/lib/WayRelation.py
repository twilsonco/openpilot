...
class WayRelation():
  ...
  def road_name(self):
    if self.name is not None:
      return f"{self.name} ({self.highway_type})" if self.highway_type is not None else self.name
    return f"{self.ref} ({self.highway_type})" if self.highway_type is not None else self.ref
