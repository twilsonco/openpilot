...
from selfdrive._c_mem import get, update, check
...
class MapD():
  ...
  def publish(self, sm):
    ...
    update('mapValid', sm.all_alive(service_list=['gpsLocationExternal']) and \
                         sm.all_valid(service_list=['gpsLocationExternal']))

    update('lastGpsSpeed', float(self.last_gps.speed))
    update('lastGpsAccuracy', float(self.last_gps.accuracy))

    if get('MapSpeedLimitEnabled', False, True):
      update('speedLimit', float(speed_limit if speed_limit is not None else 0.0))
    update('upcomingSpeedLimit', float(next_speed_limit_section.value
                                                     if next_speed_limit_section is not None else 0.0))
    update('upcomingSpeedLimitDistance', float(next_speed_limit_section.start
                                                             if next_speed_limit_section is not None else 0.0))

    update('mapTurnSpeedLimit', float(turn_speed_limit_section.value
                                                    if turn_speed_limit_section is not None else 0.0))
    update('mapTurnSpeedLimitEndDistance', float(turn_speed_limit_section.end
                                                               if turn_speed_limit_section is not None else 0.0))
    update('mapTurnSpeedLimitsAhead', [float(s.value) for s in next_turn_speed_limit_sections])
    update('mapTurnSpeedLimitsAheadDistances', [float(s.start) for s in next_turn_speed_limit_sections])

    update('mapCurrentRoadName', str(current_road_name if current_road_name is not None else ""))

    update('mapDataType', self.data_type)


# provides live map data information
def mapd_thread(sm=None):
  mapd = MapD()
