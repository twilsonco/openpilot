#!/usr/bin/env python3
# SunnyPilot custom dependencies installer: https://github.com/sunnyhaibin/sunnypilot/
import os
import sys
import errno
import shutil
...
THIRD_PARTY_DIR_C = '/data/third_party_community'
...
def install_dep(spinner):
  ...
  dup = f'cp -rf {THIRD_PARTY_DIR} {THIRD_PARTY_DIR_C}'
...
if __name__ == "__main__" and (OPSPLINE_SPEC is None or OVERPY_SPEC is None):
  spinner = Spinner()
  if os.path.exists(THIRD_PARTY_DIR_C):
    spinner.update("Loading dependencies")
    command = f'rm -rf {THIRD_PARTY_DIR}; cp -rf {THIRD_PARTY_DIR_C} {THIRD_PARTY_DIR}'
    process = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True)
    print(f"SP_LOG: Removed directory {THIRD_PARTY_DIR}")
    print(f"SP_LOG: Copied {THIRD_PARTY_DIR_C} to {THIRD_PARTY_DIR}")
  else:
