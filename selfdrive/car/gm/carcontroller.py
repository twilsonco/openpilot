    ...
    if not enabled or CS.pause_long_on_gas_press:
      ...
      if CS.one_pedal_mode_active:
        if CS.coasting_long_plan in ['cruise', 'limit']:
          apply_brake = interp(CS.vEgo, CS.one_pedal_mode_stop_apply_brake_bp[CS.one_pedal_brake_mode], CS.one_pedal_mode_stop_apply_brake_v[CS.one_pedal_brake_mode])
          time_since_brake = sec_since_boot() - CS.one_pedal_mode_last_gas_press_t
          apply_brake *= interp(time_since_brake, CS.one_pedal_mode_ramp_time_bp, CS.one_pedal_mode_ramp_time_v) if CS.one_pedal_brake_mode < 2 else 1.
