...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators,
    ...
    if not enabled or CS.pause_long_on_gas_press:
      ...
      if (CS.coasting_lead_v - CS.vEgo < CS.lead_v_rel_long_lockout_bp[-1] or CS.coasting_lead_v < CS.lead_v_long_lockout_bp[-1]):
        lead_long_lockout_factor = max(interp(CS.coasting_lead_v - CS.vEgo, CS.lead_v_rel_long_lockout_bp, CS.lead_v_rel_long_lockout_v), interp(CS.coasting_lead_v, CS.lead_v_long_lockout_bp, CS.lead_v_long_lockout_v))
      else:
        lead_long_lockout_factor =  0. # 1.0 means regular braking logic is completely unaltered, 0.0 means no cruise braking
      
      if lead_long_lockout_factor < 1.:
        if (CS.one_pedal_mode_active or CS.coast_one_pedal_mode_active):
          ...
          if CS.coasting_long_plan in ['cruise', 'limit'] and apply_gas < P.ZERO_GAS or apply_brake > 0.:
            if apply_brake > 0.:
              ...
              apply_brake = max([apply_brake * lead_long_lockout_factor, coast_brake, over_speed_brake])
          ...
          if CS.coasting_long_plan in ['cruise', 'limit'] and apply_brake > 0.:
            ...
            apply_brake *= over_speed_factor * lead_long_lockout_factor
